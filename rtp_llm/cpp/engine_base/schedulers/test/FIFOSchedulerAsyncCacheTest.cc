...
protected:
    ...
    void SetUp() override {
        ...
        cache_manager_ = std::make_shared<KVCacheManager>(cache_config_, device_);
        ASSERT_TRUE(cache_manager_->init());
    }

    void setupMockCoordinator() {
        mock_coord_ = std::make_shared<NiceMock<MockKVCacheConnectorCoordinator>>(
...
TEST_F(FIFOSchedulerAsyncCacheTest, testScheduleNew_WithReuseCache_EntersLoadingCache) {
    ...
    EXPECT_CALL(*mock_coord_, asyncRead(_))
        .WillOnce(Return(std::static_pointer_cast<AsyncContext>(pending_ctx)));
...
TEST_F(FIFOSchedulerAsyncCacheTest, testEvaluateLoadingCache_LoadDone_MovesToRunning) {
    ...
    EXPECT_CALL(*mock_coord_, asyncRead(_))
        .WillOnce(Return(std::static_pointer_cast<AsyncContext>(mock_ctx)));
...
TEST_F(FIFOSchedulerAsyncCacheTest, testLoadingCacheStreams_CountedInBatchLimit) {
    ...
    auto pending_ctx = createPendingAsyncContext();
    EXPECT_CALL(*mock_coord_, asyncRead(_))
        .WillRepeatedly(Return(std::static_pointer_cast<AsyncContext>(pending_ctx)));
...
TEST_F(FIFOSchedulerAsyncCacheTest, testMixedAsyncAndDirectStreams) {
    ...
    // Stream2: no reuse cache, goes directly to RUNNING
    ...
    auto result = scheduler->schedule();
    ASSERT_TRUE(result.ok());
    // stream1 -> LOADING_CACHE, stream2 -> RUNNING
    ASSERT_EQ(result.value().size(), 1);
    ASSERT_TRUE(stream1->loadingCache());
    ASSERT_TRUE(stream2->running());
    ASSERT_EQ(scheduler->loading_cache_streams_.size(), 1);
    ASSERT_EQ(scheduler->runningStreamsSize(), 1);
}
...
TEST_F(FIFOSchedulerAsyncCacheTest, testScheduleOrdering_LoadDoneStreamsAtWaitingHead) {
    ...
    // stream1 should have been scheduled (it was at head of waiting queue)
    ASSERT_TRUE(stream1->running());
}
